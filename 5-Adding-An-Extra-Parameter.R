#Adding an extra parameter to the topology.
#This R script contains code to perform ABC for model with 3 populations, 2 admixture events, 5 parameters.
...
paramSims<-data.frame(delta1Sims,deltamSims,delta2MinusmSims,alphaSims,betaSims)
names(paramSims)<-c("delta1Sims", "deltamSims","delta2MinusmSims", "alphaSims","betaSims")
...
for (val in 1:simsSize) {
  ...
  VarX2=mySigmaSq*(currentAlpha^2*(currentDelta1+currentDeltam)+(1-currentAlpha)^2*(currentDelta1+currentDeltam)+currentDelta2Minusm)
  ...
  CovX2X3=(1-currentAlpha)*(1-currentBeta)*mySigmaSq*currentDelta1+currentBeta*VarX2
  
  VarX2B=mySigmaSq/(2*myLambda)*(
    currentAlpha^2*(exp(2*myLambda*(currentDelta1+currentDeltam))-1)+
      (1-myAlpha)^2*(exp(2*myLambda*(currentDelta1+currentDeltam))-1)+
      exp(2*myLambda*(currentDelta1+currentDeltam+currentDelta2Minusm))-
  ...
  CovX2X3B=(1-currentAlpha)*(1-currentBeta)*mySigmaSq/(2*myLambda)*(exp(2*myLambda*currentDelta1)-1)
  ...
  currentCovMatrix=matrix(c(mySigmaSq*(currentDelta1+currentDeltam+currentDelta2Minusm),
                            ...
                            CovX1X3, 
                            CovX2X3, 
                            currentBeta^2*VarX2+(1-currentBeta)^2*mySigmaSq*(currentDelta1+currentDeltam+currentDelta2Minusm)+2*currentBeta*(1-currentBeta)*(1-currentAlpha)*mySigmaSq*currentDelta1),nrow=3,ncol=3,byrow = TRUE)
  ...
  currentCovMatrixB <- matrix(c(mySigmaSq/(2*myLambda)*(exp(2*myLambda*(currentDelta1+currentDeltam+currentDelta2Minusm))-1),
                                ...
                                CovX1X3B, 
                                CovX2X3B, 
                                currentBeta^2*VarX2B+(1-currentBeta)^2*mySigmaSq/(2*myLambda)*(exp(2*myLambda*(currentDelta1+currentDeltam+currentDelta2Minusm))-1)+2*currentBeta*(1-currentBeta)*(1-currentAlpha)*mySigmaSq/(2*myLambda)*(exp(2*myLambda*currentDelta1)-1)
...
VarX2=mySigmaSq*(myAlpha^2*(myDelta1+myDeltam)+(1-myAlpha)^2*(myDelta1+myDeltam)+myDelta2Minusm)
CovX1X2=myAlpha*mySigmaSq*(myDelta1+myDeltam)
CovX1X3=myAlpha*myBeta*mySigmaSq*(myDelta1+myDeltam)
CovX2X3=(1-myAlpha)*(1-myBeta)*mySigmaSq*myDelta1+myBeta*VarX2

VarX2B=mySigmaSq/(2*myLambda)*(
  myAlpha^2*(exp(2*myLambda*(myDelta1+myDeltam))-1)+
    (1-myAlpha)^2*(exp(2*myLambda*(myDelta1+myDeltam))-1)+
    exp(2*myLambda*(myDelta1+myDeltam+myDelta2Minusm))-
...
CovX2X3B=(1-myAlpha)*(1-myBeta)*mySigmaSq/(2*myLambda)*(exp(2*myLambda*myDelta1)-1)+myBeta*VarX2B
...
sample_covariance_matrix <- matrix(c(mySigmaSq*(myDelta1+myDeltam+myDelta2Minusm),
                                     ...
                                     CovX1X3, 
                                     CovX2X3, 
                                     myBeta^2*VarX2+(1-myBeta)^2*mySigmaSq*(myDelta1+myDeltam+myDelta2Minusm)+2*myBeta*(1-myBeta)*(1-myAlpha)*mySigmaSq*myDelta1),nrow=3,ncol=3,byrow = TRUE)
...
sample_covariance_matrixB <- matrix(c(mySigmaSq/(2*myLambda)*(exp(2*myLambda*(myDelta1+myDeltam+myDelta2Minusm))-1),
                                      ...
                                      CovX1X3B, 
                                      CovX2X3B, 
                                      myBeta^2*VarX2B+(1-myBeta)^2*mySigmaSq/(2*myLambda)*(exp(2*myLambda*(myDelta1+myDeltam+myDelta2Minusm))-1)+2*myBeta*(1-myBeta)*(1-myAlpha)*mySigmaSq/(2*myLambda)*(exp(2*myLambda*myDelta1)-1)
),nrow=3,ncol=3,byrow = TRUE)
